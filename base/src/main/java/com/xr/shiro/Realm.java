...
//import com.xr.exception.ProjectException;
...
public class Realm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        List<SysRole> roless = roleService.findUserByRoless(sysUser.getUsername());
        // 根据身份信息获取角色信息，权限信息
        for (SysRole role : roless) {
            simpleAuthorizationInfo.addRole(role.getName());
            System.out.println("roleName:"+role.getName());
            // 根据身份信息获取权限信息
            List<SysMenu> menus = menuService.findRoleMenus(sysUser.getUsername());
            for (SysMenu m : menus) {
                if(m.getPerms()!=null) {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token)
        ...
        try {
            ...
            SysUser sysUser = usersService.findUserByUserName(username);
            if (sysUser == null) {
                return null;
            }
